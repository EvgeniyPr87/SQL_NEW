...
--	DECLARE @start_date			AS	DATE	=	dbo.GetNextDate(@group_name);
	...
	BEGIN
		IF dbo.GetLastDate(@group_name) IS NOT NULL SET @date = dbo.GetNextWorkDate(@group_name);
		SET @time = @start_time;
...
--	WHILE @lesson_number<=@number_of_lessons
--	BEGIN
--		IF NOT EXISTS(SELECT 1  FROM Schedule WHERE [group] =@group_id AND [date]=@date AND [time]=@time )
--		BEGIN
